...
registerBtn.addEventListener("click", async function (event) {
    ...
    try {
        ...
        if (registerResponse.ok) {
            ...
            alert("Регистрация прошла успешно!"); // Твой текст при новой регистрации
        ...
        if (registerResponse.status === 400 || registerResponse.status === 409 || errorData.detail === "User already exists") {
            console.log("Юзер уже есть в БД. Молча переключаемся на авторизацию...");

            // Переводим данные в формат x-www-form-urlencoded 
